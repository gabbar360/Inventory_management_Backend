...
class PaymentsReceivedController {
  ...
  static async getById(req, res) {
    try {
      const { id } = req.params;
      const payment = await PaymentsReceivedService.getById(id);
      return sendResponse(res, 200, true, payment, 'Payment received retrieved successfully');
    } catch (error) {
  ...
  static async create(req, res) {
    try {
      const payment = await PaymentsReceivedService.create(req.body);
      return sendResponse(res, 201, true, payment, 'Payment received created successfully');
    } catch (error) {
  ...
  static async update(req, res) {
    try {
      const { id } = req.params;
      const payment = await PaymentsReceivedService.update(id, req.body);
      return sendResponse(res, 200, true, payment, 'Payment received updated successfully');
    } catch (error) {
      return sendError(res, 400, error.message);
    }
  }
  ...
  static async applyCredits(req, res) {
    try {
      const result = await PaymentsReceivedService.applyCredits(req.body);
  ...
  static async delete(req, res) {
    try {
      const { id } = req.params;
      const result = await PaymentsReceivedService.delete(id);
      return sendResponse(res, 200, true, result, 'Payment received deleted successfully');
    } catch (error) {
      return sendError(res, 400, error.message);
    }
  }
  ...
  static async generatePDF(req, res) {
    try {
      ...
      const templatePath = path.join(__dirname, '../templates/paymentReceivedTemplate.ejs');
...
module.exports = { PaymentsReceivedController };
