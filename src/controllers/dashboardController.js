...
class DashboardController {
  static async getKPIs(req, res) {
    try {
      ...
      const result = await DashboardService.getKPIs(period, dateFrom, dateTo);
  ...
  static async getRevenueChart(req, res) {
    try {
      const { period, dateFrom, dateTo } = req.query;
      const result = await DashboardService.getRevenueChart(period, dateFrom, dateTo);
  ...
  static async getTopProducts(req, res) {
    try {
      ...
      const result = await DashboardService.getTopProducts(parseInt(limit) || 10, dateFrom, dateTo);
  ...
  static async getTopCustomers(req, res) {
    try {
      const { limit, dateFrom, dateTo } = req.query;
      const result = await DashboardService.getTopCustomers(parseInt(limit) || 10, dateFrom, dateTo);
  ...
  static async getPerformanceMetrics(req, res) {
    try {
      const { period, dateFrom, dateTo } = req.query;
      const result = await DashboardService.getPerformanceMetrics(period, dateFrom, dateTo);
