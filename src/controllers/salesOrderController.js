...
const createSalesOrder = async (req, res) => {
  try {
    const { customerId, orderDate, items, saleType, status, totalAmount, notes, reference, expectedShipmentDate, placeOfSupply, deliveryMethod, adjustment, amountReceived } = req.body;
    if (!customerId || !orderDate || !items || items.length === 0)
      return res.status(400).json({ success: false, error: 'Missing required fields' });

    const order = await salesOrderService.createSalesOrder({ customerId, orderDate, items, saleType, status, totalAmount, notes, reference, expectedShipmentDate, placeOfSupply, deliveryMethod, adjustment, amountReceived });
